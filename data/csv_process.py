...
def prepare_image_df(dataset_folder_path):
    images = glob(dataset_folder_path + '/*/*/*/*.png')
    image_df = pd.DataFrame(images, columns=["image_path"])

    image_df["case"] = image_df["image_path"].str.split("\\").str[1]
    image_df["day"] = image_df["image_path"].str.split("\\").str[2].str.split('_').str[1]
    image_df["slice_id"] = image_df["image_path"].str.split("\\").str[4].str.split('_').str[1]

    image_df["pic_info"] = image_df["image_path"].str.split("\\").str[4]
...
if __name__ == '__main__':
    # usage: python csv_process.py [label_csv_path] [dataset_folder_path] [output_csv_name]
    label_csv_path = sys.argv[1]
    dataset_folder_path = sys.argv[2]
    output_csv_path = sys.argv[3]
    label_df = prepare_label_df(label_csv_path)
    image_df = prepare_image_df(dataset_folder_path)
