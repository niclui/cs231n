...
class CLearn(nn.Module):
    ...
    def __init__(self, n_channels, feature_dim):
        super().__init__()
        self.g = nn.Sequential(nn.Linear(16 * n_channels, 16 * n_channels, bias=False),
                                nn.BatchNorm1d(16 * n_channels),
                                nn.ReLU(inplace=True), 
                                nn.Linear(16 * n_channels, feature_dim, bias=True))
...
class CLUNet(nn.Module):
    def __init__(self, in_channels, n_classes, n_channels, pretraining = False):
        ...
        self.clearn = CLearn(n_channels, feature_dim = 16)
    ...
    def forward(self, x, aux = None, pretraining = False):
        ...
        if aux is not None:
            clearn_out = self.clearn(self.pooling(x6).squeeze())
